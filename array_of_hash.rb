class Student 
  ...
  def to_hash
    ...
    self.instance_variables.each do |a|
      hash[a.to_s.gsub('@', '').to_sym] = self.instance_variable_get(a)
    end
    return hash
  end
end
  ...
  def get_student_details(grade="", students)
    ...
    student = Student.new({ name: name,contact: contact,grade: grade }).to_hash
    system("clear")
  ...
  def display_gradewise_soted_list_of_student(students)
    system("clear")
    temp = students.sort_by{ |student| [student[:grade],student[:name]] }
